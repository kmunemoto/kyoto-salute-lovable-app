    ...
    <section id="plan" className="section-padding section-dark">
      <div className="max-w-7xl mx-auto">
        ...
        */}
        ...
        {/*
          オプション（追加セッション・パーソナルストレッチ）。プランに追加する形なので、
          プランカードと同格に見せないよう、グリッドの外に一段控えめな帯として置く。
          予約の導線は各プランのCTAに集約されているため、ここには専用のボタンを置かない。
        */}
        <div className="mt-10 max-w-2xl mx-auto space-y-4">
          {p.options.map((o) => (
            <div key={o.name} className="rounded-sm p-6 bg-white border border-border">
              <p className="text-gold text-xs tracking-[0.2em] uppercase mb-2 font-body">{o.label}</p>
              <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1 mb-2">
                <h3 className="text-gym-dark-foreground font-body font-semibold text-lg">{o.name}</h3>
                <span className="text-gym-dark-foreground/50 text-sm font-body">{o.duration}</span>
              </div>
              <div className="mb-3">
                <span className="text-2xl font-body font-bold text-gold">{o.price}</span>
                <span className="text-gym-dark-foreground/50 text-sm font-body ml-2">{o.unit}</span>
              </div>
              <p className="text-gym-dark-foreground/70 text-sm font-body leading-relaxed">{o.description}</p>
              <p className="text-gym-dark-foreground/70 text-sm font-body leading-relaxed mt-2">{o.benefit}</p>
              <p className="text-gym-dark-foreground/50 text-xs font-body mt-3">{o.booking}</p>
            </div>
          ))}
        </div>
      </div>
